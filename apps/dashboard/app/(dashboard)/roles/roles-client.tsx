...
/**
 * Client component for roles page.
 * Receives server-fetched data for instant render.
 * @param props - initialRoles, initialCollections, permissions
 */
export const RolesClient = ({
	...
	const myCanCreate = hasPerm(permissions, 'roles', 'create');
	const myCanUpdate = hasPerm(permissions, 'roles', 'update');
	const myCanDelete = hasPerm(permissions, 'roles', 'delete');

	const { data: freshRoles, refetch: refetchRoles } = useCachedFetch<Role[]>('/api/roles');
	const { data: freshColData } = useCachedFetch<ContentType[]>('/api/collections');
	const roles = freshRoles ?? initialRoles;
	const contentTypes = (freshColData ?? initialCollections).map((c) => ({ slug: c.slug, name: c.name }));
